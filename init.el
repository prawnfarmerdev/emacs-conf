;;; init.el --- Emacs Configuration with Tmux/Vim-style keybindings
...
(defun my/fzf-project-dirs ()
  ...
  (interactive)
  (let* ((dirs (list (expand-file-name "~/files")
         ...
         (fzf-cmd (format "{ %s; } | fzf" combined-cmd))
...
(use-package vterm
  ...
  (add-hook 'vterm-mode-hook
            (lambda ()
              (font-lock-mode 1)
              (face-remap-add-relative 'default :background (face-background 'default)))))

;;==============================================================================
;; HELPER FUNCTIONS
;;==============================================================================

;; Default value for consult-ripgrep-args (will be overridden by consult config)
...
(use-package general
  ...
  (my-leader-def
    ...
    "t" '(my/open-vterm-here :which-key "terminal (tmux)")
...
;; Close use-package general block
)
